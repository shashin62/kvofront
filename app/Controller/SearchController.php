	...
	private function _getLists($datas, $groupIds, $loggedInId)
	{
		...
		foreach($datas as $dataKey=> $result) 
		{
			...
			if ( $loggedinId  != $result['p']['id'] && $result['p']['tree_level'] == '') {
				if( $loggedinId == $result['p']['f_id'] || $loggedinId == $result['p']['m_id'] || $loggedinId == $result['p']['partner_id']
                || in_array($loggedinId,  explode(',', $result[0]['brothers'])) || in_array($loggedinId,  explode(',', $result[0]['sisters']))
                || in_array($loggedinId,  explode(',', $result[0]['childrens']))) 
				{
			...
			if (file_exists($_SERVER["DOCUMENT_ROOT"] . '/people_images/' . $result['p']['id'] . '.' . $result['image']['ext']) === true) {
				$lists[$result['id']]['r'] = $result['p']['id'] . '.' . $result['image']['ext'];
			} else {
				$lists[$result['id']]['r'] = '';
			}
	...
	*/
    ...
    private function __getRelationShipText($tmpArray1, $levels, $searchedId) {
        ...
        foreach (array_filter($levels) as $k => $c) {
            ...
            if (in_array($c, $tmpArray1[$id]) || in_array($c, $tmpArray1[$id]['sisters']) 
                    || in_array($c, $tmpArray1[$id]['brothers']) 
                    || in_array($c, $tmpArray1[$id]['childs_f'])
                    || in_array($c, $tmpArray1[$id]['childs_m'])
            ) {
                ...
                switch ($key)
        {
            case 'f_id' :
                ...
                $text[] = '<a href="javascript: search('.$tmpArray1[$id]['id'].')" style="width:50px;">' . $pic . $tmpArray1[$c]['first_name'] . ' ' . $tmpArray1[$c]['last_name'] . '</a>';
                break;
            case 'm_id':
                if ($tmpArray1[$id]['gender'] == 'Male') {
                    $text[] = '<span style="font-size:12px;">&nbsp;&nbsp;--<b>Son of &nbsp;</b>-->&nbsp;&nbsp;</span>';
