...
else
{
    ...
    if(count($missing_aco_nodes) > 0)
    {
        ...
        echo '<p>';
        echo $this->Html->link($this->Html->image('/img/design/add.png') . ' ' . __d('acl', 'Build'), '/admin/acl/acos/build_acl/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo $this->Html->image('/img/design/tick.png') . ' ' . __d('acl', 'There is no ACO node to create');
        echo '</p>';
    }
}
