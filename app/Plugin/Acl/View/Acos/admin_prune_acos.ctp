...
else
{
    ...
    if(count($nodes_to_prune) > 0)
    {
        ...
        echo '<p>';
        echo $this->Html->link($this->Html->image('/img/design/clean.png') . ' ' . __d('acl', 'Prune'), '/admin/acl/acos/prune_acos/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo $this->Html->image('/img/design/tick.png') . ' ' . __d('acl', 'There is no ACO node to delete');
        echo '</p>';
    }
}
