...
else
{
    ...
    if($has_aco_to_sync)
    {
        ...
        echo '<p>';
        echo $this->Html->link($this->Html->image('/img/design/sync.png') . ' ' . __d('acl', 'Synchronize'), '/admin/acl/acos/synchronize/run', array('escape' => false));
        echo '</p>';
    }
    else
    {
        echo '<p style="font-style:italic;">';
        echo $this->Html->image('/img/design/tick.png') . ' ' . __d('acl', 'The ACO datatable is already synchronized');
        echo '</p>';
    }
}
