...
?>
    ...
    foreach($users as $user)
    {
        ...
        echo '  <td>' . $this->Html->link($this->Html->image('/img/design/lock.png'), $link, array('alt' => $title, 'title' => $title, 'escape' => false)) . '</td>';
...
?>
    ...
    <tr>
    	...
    	foreach($roles as $role)
    	{
    	    ...
    	    {
    	        echo $this->Html->image('/img/design/tick.png');
    	    }
    	    else
    	    {
    	    	$title = __d('acl', 'Update the user role');
    	        echo $this->Html->link($this->Html->image('/img/design/tick_disabled.png'), array('plugin' => 'acl', 'controller' => 'aros', 'action' => 'update_user_role', 'user' => $user[$user_model_name][$user_pk_name], 'role' => $role[$role_model_name][$role_pk_name]), array('title' => $title, 'alt' => $title, 'escape' => false));
    	    }
	...
	if($user_has_specific_permissions)
	{
    	...
    	echo $this->Html->image('/img/design/bulb24.png') . __d('acl', 'This user has specific permissions');
    	echo ' (';
    	echo $this->Html->link($this->Html->image('/img/design/cross2.png', array('style' => 'vertical-align:middle;')) . ' ' . __d('acl', 'Clear'), '/admin/acl/aros/clear_user_specific_permissions/' . $user[$user_model_name][$user_pk_name], array('confirm' => __d('acl', 'Are you sure you want to clear the permissions specific to this user ?'), 'escape' => false));
    	echo ')';
    	echo '<div class="separator"></div>';
	}
	?>
	...
	if(isset($actions['app']) && is_array($actions['app']))
	{
    	foreach($actions['app'] as $controller_name => $ctrl_infos)
    	{
    		...
    		foreach($ctrl_infos as $ctrl_info)
    		{
        		...
        		{
        		    echo $this->Html->image('/img/design/important16.png');
        		}
        		
    	    	echo '</span>';
    	    	
    	    	echo ' ';
    	    	echo $this->Html->image('/img/ajax/waiting16.gif', array('id' => 'right__' . $user[$user_model_name][$user_pk_name] . '_' . $controller_name . '_' . $ctrl_info['name'] . '_spinner', 'style' => 'display:none;'));
    	...
    	foreach($actions['plugin'] as $plugin_name => $plugin_ctrler_infos)
    	{
    	    ...
    	    foreach($plugin_ctrler_infos as $plugin_ctrler_name => $plugin_methods)
    	    {
    	        ...
    	        foreach($plugin_methods as $method)
    	        {
    		    	...
    		    	echo ' ';
    		    	echo $this->Html->image('/img/ajax/waiting16.gif', array('id' => 'right_' . $plugin_name . '_' . $user[$user_model_name][$user_pk_name] . '_' . $plugin_ctrler_name . '_' . $method['name'] . '_spinner', 'style' => 'display:none;'));
